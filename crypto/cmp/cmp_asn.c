...
#include "cmp_local.h"
#include "internal/crmf.h" /* for struct ossl_crmf_attributetypeandvalue_st */
...
*OSSL_CMP_ITAV_new0_certReqTemplate(OSSL_CRMF_CERTTEMPLATE *certTemplate,
                                    STACK_OF_OSSL_CMP_ATAV *keySpec)
{
    OSSL_CMP_ITAV *itav;
    ...
    tmpl->keySpec = (STACK_OF(OSSL_CMP_ATAV) *)keySpec;
    return itav;
}
                                       ...
                                       STACK_OF_OSSL_CMP_ATAV **keySpec)
{
    ...
    if (keySpec != NULL && req->keySpec != NULL) {
        int i, n = sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_num(req->keySpec);

        *keySpec = (STACK_OF_OSSL_CMP_ATAV *)sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_new_reserve(NULL, n);
        if (*keySpec == NULL)
            return 0;
        for (i = 0; i < n; i++) {
            OSSL_CMP_ATAV *atav =
                sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_value(req->keySpec, i);
            ...
            sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_push((STACK_OF(OSSL_CMP_ATAV) *)*keySpec, atav);
 ...
 err:
    ...
    *certTemplate = NULL;
    sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_pop_free((STACK_OF(OSSL_CMP_ATAV) *)*keySpec, OSSL_CMP_ATAV_free);
    if (keySpec != NULL)
        *keySpec = NULL;
    return 0;
}
